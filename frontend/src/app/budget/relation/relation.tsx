...
export default function Relation({data, onOk, onCancel}: {
    data: RelationData,
    onOk?: (b: boolean) => void,
    onCancel?: () => void
}) {
    ...
    useEffect(() => {
        ...
        api().url("/api/program?no_pagination=true&year=" + Year).get().json<any[]>().then(r => {
            // console.log(r)
            set_programs(r)
        })
        api().url("/api/budget_row?no_pagination=true&year=" + Year).get().json<any[]>().then(r => {
            // console.log(r)
            set_budget_row(r)
        })
    ...
    return (
        ...
        >
            ...
            <Form.Item name="Location" label={"انتخاب محل هزینه"} rules={[
                {
                    required: true
                },
            ]}>
